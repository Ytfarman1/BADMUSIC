import asyncio, httpx, os, re, yt_dlp, aiohttp
import urllib.parse
...
async def get_stream_url(query: str, video: bool = False):
    """
    Get audio/video stream URL from the new API
    """
...
class YouTubeAPI:
    ...
    async def video(self, link: str, videoid: Union[bool, str] = None):
        """
        Get video stream URL using the new API
        """
        if videoid:
            link = self.base + link
        if "&" in link:
    ...
    async def audio(self, link: str, videoid: Union[bool, str] = None):
        """
        Get audio stream URL using the new API
        Note: The API returns the same stream URL for both audio and video
        You may need to adjust based on the actual API behavior
        """
        if videoid:
            link = self.base + link
        if "&" in link:
    ...
    async def download(
        ...
        """
        Download method using the new API for simple audio/video streams
        """
        if videoid:
            link = self.base + link
